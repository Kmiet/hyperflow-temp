...
//   * context
//   * cb
async function k8sCommandGroup(bufferItems, node_selector='') {
  ...
  console.log("k8sCommandGroup started, time:", startTime, "job_count:", bufferItems.length, node_selector !== '' ? `node: ${node_selector}` : '');
  ...
  console.log("Ending k8sCommandGroup function, time:", endTime, "exit codes:", jobExitCodes, "job_count:", bufferItems.length, node_selector !== '' ? `node: ${node_selector}` : '');
...
async function k8sCommand(ins, outs, context, cb) {
  ...
  await scheduler.addTaskItem(item, k8sCommandGroup);
