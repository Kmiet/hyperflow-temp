...
var submitK8sJob = async(kubeconfig, jobArr, taskIdArr, contextArr, customParams, restartFn) => {
  ...
  var awaitJob = async(taskId) => {
    ...
    let localProcId = taskId.split(':')[2];
    console.log('[k8s][Submit][TaskCompleted]', 'Notifying completion of: ', context.procId, localProcId, Date.now());
    scheduler.notifyTaskCompletion(context.appId, localProcId);
